...
namespace com.ootii.Graphics
{
    ...
    public class GraphicsManager : MonoBehaviour
    {
        ...
        public static void DrawSolidCone(Vector3 rPosition, Vector3 rDirection, float rDistance, float rRadius, Color rColor, Transform rTransform = null, float rDuration = 0f)
        {
            ...
            Vector3 lEndCenter = rPosition + (rDirection * rDistance);
            ...
            for (int i = 0; i < lCount; i++)
            {
                mVectors1.Add(lEndCenter + lSurfacePoint);
                lSurfacePoint = lRotation * lSurfacePoint;
            }
            ...
            for (int i = 1; i < lCount; i++)
            {
                // End circle
                DrawLine(mVectors1[i - 1], mVectors1[i], rColor, rTransform, rDuration);
                DrawTriangle(lEndCenter, mVectors1[i - 1], mVectors1[i], lColor, rTransform, rDuration);

                // Cone wall
                DrawTriangle(rPosition, mVectors1[i - 1], mVectors1[i], lColor, rTransform, rDuration);
            }
        }
        ...
        public static void DrawSphere(Vector3 rCenter, float rRadius, Color rColor, float rDuration = 0f)
        {
            ...
            DrawArc(rCenter, lForward, lUp, 360f, rRadius, rColor, null, rDuration);
            DrawArc(rCenter, lUp, lRight, 360f, rRadius, rColor, null, rDuration);
            DrawArc(rCenter, lRight, -lUp, 360f, rRadius, rColor, null, rDuration);
        }
