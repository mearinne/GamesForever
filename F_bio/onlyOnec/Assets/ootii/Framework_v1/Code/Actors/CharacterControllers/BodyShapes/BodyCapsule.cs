...
namespace com.ootii.Actors
{
    ...
    public class BodyCapsule : BodyShape
    {
        ...
        public override BodyShapeHit[] CollisionCastAll(Vector3 rPositionDelta, Vector3 rDirection, float rDistance, int rLayerMask)
        {
            ...
            for (int i = 0; i < lHitCount; i++)
            {
                ...
                if (lBodyShapeHit != null)
                {
                    ...
                    // TRT 03/12/17 - When colliding with edges, we sometimes got odd hit normals. Meaning Unity
                    // will swap back and forth between different normals. So, we want to use our own calculation.
        ...
        public override void CreateUnityColliders()
        {
            ...
            UnityEngine.Object lPrefabParent = UnityEditor.PrefabUtility.GetCorrespondingObjectFromSource(_Parent.gameObject);
            ...
            if (UnityEditor.EditorApplication.isPlaying)
            {
                ...
                else
                {
                    ...
                    if (lLocalEndPosition == lLocalStartPosition)
                    {
                        if (mColliders == null || mColliders.Length != 1) { mColliders = new UnityEngine.Collider[1]; }

                        SphereCollider lStartCollider = lTransform.gameObject.AddComponent<SphereCollider>();
                        lStartCollider.radius = _Radius;
                        lStartCollider.center = lLocalStartPosition;
                        mColliders[0] = lStartCollider;
                    }
                    else
                    {
                        ...
                        if (mColliders == null || mColliders.Length != lCount) { mColliders = new UnityEngine.Collider[lCount + 2]; }

                        // Add one at the start point
                        SphereCollider lStartCollider = lTransform.gameObject.AddComponent<SphereCollider>();
                        lStartCollider.radius = _Radius;
                        lStartCollider.center = lLocalStartPosition;
                        mColliders[0] = lStartCollider;

                        // Add one at the end point
                        ...
                        mColliders[mColliders.Length - 1] = lEndCollider;
                        ...
                        for (int i = 1; i < mColliders.Length - 1; i++)
                        {
                            ...
                            lCollider.radius = _Radius;
                            lCollider.center = lLocalStartPosition + (lToEnd.normalized * (_Radius * i));
        ...
        public override bool OnInspectorGUI(UnityEngine.Object rTarget)
        {
            ...
            if (EditorHelper.ObjectField<Transform>("Bottom Transform", "", Transform, rTarget))
            {
                lIsDirty = true;
                Transform = EditorHelper.FieldObjectValue as Transform;
            }

            if (EditorHelper.Vector3Field("Bottom Offset", "", Offset, rTarget))
            {
                lIsDirty = true;
            ...
            if (EditorHelper.ObjectField<Transform>("Top Transform", "", EndTransform, rTarget))
            {
                lIsDirty = true;
                EndTransform = EditorHelper.FieldObjectValue as Transform;
            }

            if (EditorHelper.Vector3Field("Top Offset", "", EndOffset, rTarget))
            {
                lIsDirty = true;
                EndOffset = EditorHelper.FieldVector3Value;
            }
